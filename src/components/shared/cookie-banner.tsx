...
export function CookieBanner() {
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 border-t border-[var(--color-border)] bg-white/95 backdrop-blur-lg px-6 py-4">
      <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
        <Cookie className="w-5 h-5 text-[var(--color-text-secondary)] shrink-0 hidden sm:block" />
        ...
        <button
          ...
          className="shrink-0 px-5 py-2 text-sm font-medium text-[var(--color-text)] bg-[var(--color-bg-surface)] hover:bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-lg transition-colors"
        >
          Compris
        </button>
      </div>
