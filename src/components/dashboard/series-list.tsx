  ...
  return (
    <div className={className}>
      ...
      {series.length > 0 ? (
          ...
          {series.map((s, index) => (
            ...
            >
              <Link href={`/dashboard/results/series?job=${jobId}&series=${encodeURIComponent(s.series_id)}`}>
                <div className="group flex items-center justify-between p-3 lg:p-4 rounded-xl bg-[var(--color-bg-surface)] hover:bg-[var(--color-bg-surface)] border border-transparent hover:border-[var(--color-border)] transition-all gap-2">
                  <div className="flex items-center gap-2 lg:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <BadgeWithTooltip tooltip={GLOSSARY.abc}>
                          <span
                            className={cn(
                              ...
                              classColors[s.abc_class] ?? "bg-zinc-500/20 text-[var(--color-text-secondary)] border-zinc-500/30"
                            )}
                          >
                            {s.abc_class}
                          </span>
                        ...
                        <span className="text-xs text-[var(--color-text-tertiary)] truncate hidden lg:inline">
                  ...
                  <div className="flex items-center gap-2 lg:gap-4 shrink-0">
                    ...
                    <ArrowRight className="w-4 h-4 text-[var(--color-text-tertiary)] group-hover:text-[var(--color-text-secondary)] transition-colors hidden lg:block" />
                  </div>
                </div>
              </Link>
            </motion.div>
        ...
        <div className="text-center py-8 text-[var(--color-text-tertiary)]">{emptyMessage}</div>
      )}
    </div>
  );
}
