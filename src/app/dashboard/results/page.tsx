...
// Couleurs pour la matrice ABC/XYZ
function getMatrixCellClass(row: string, col: string): string {
  // A/X = emerald, B/Y = amber, C/Z = red
  if (row === "A" && col === "X") return "bg-emerald-500/30 text-emerald-300";
  if (row === "A" && col === "Y") return "bg-emerald-500/20 text-emerald-400";
  if (row === "A" && col === "Z") return "bg-amber-500/20 text-amber-400";
  if (row === "B" && col === "X") return "bg-emerald-500/20 text-emerald-400";
  if (row === "B" && col === "Y") return "bg-amber-500/20 text-amber-400";
  if (row === "B" && col === "Z") return "bg-red-500/20 text-red-400";
  if (row === "C" && col === "X") return "bg-amber-500/20 text-amber-400";
  if (row === "C" && col === "Y") return "bg-red-500/20 text-red-400";
  if (row === "C" && col === "Z") return "bg-red-500/30 text-red-300";
        ...
        <div className="flex gap-3">
          ...
          {recentJobs.length > 1 && (
            <select
              ...
              className="px-4 py-2 bg-zinc-900/50 border border-white/[0.08] rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            >
              {recentJobs.map((job) => (
        ...
        <>
          ...
          {!loadingData && activeTab === "classification" && (
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-zinc-900/50 rounded-xl border border-white/[0.08] p-6">
                <h3 className="text-base font-semibold text-white mb-5">Classification ABC</h3>
                ...
                {jobData?.abcXyzMatrix && jobData.abcXyzMatrix.some(m => m.count > 0) ? (
                  <>
                    <div className="grid grid-cols-4 gap-1">
                      <div />
                      {["X", "Y", "Z"].map((col) => (
                        <div key={col} className="text-center p-2 font-semibold text-sm text-zinc-300 bg-white/5 rounded-md">
                          {col}
                        </div>
                      ))}
                      {(["A", "B", "C"] as const).map((row) => (
                        <Fragment key={row}>
                          <div className="p-2 font-semibold text-sm text-zinc-300 bg-white/5 rounded-md flex items-center justify-center">{row}</div>
                          {(["X", "Y", "Z"] as const).map((col) => {
                            ...
                            return (
                              <div
                                key={`${row}-${col}`}
                                className={`rounded-md p-4 text-center text-sm font-semibold ${getMatrixCellClass(row, col)}`}
                              >
                                {value}
                              </div>
                            );
